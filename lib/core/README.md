...
- **Protocol runtime context** - Payloads may depend on explicit runtime state during encoding and decoding, and generated protocols carry shared size limits through `ProtocolSchema`.
...
If your payload needs runtime state during encoding or decoding, see the `Protocol Context` section below.
...
If you need runtime payload state, see the `Protocol Context` section.
...
For generated protocols these values are configured through `brec::generate!()`.
...
If you need the runtime-state side of this model first, see [Protocol Context](#protocol-context).
