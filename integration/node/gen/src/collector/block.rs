...
pub fn generate_impl(blocks: &[&Block]) -> Result<TokenStream, E> {
    ...
    for blk in blocks.iter() {
        ...
        to_wrapped.push(quote! {
            Block::#fullname(block) => {
                let value = brec::napi_feat::NapiObject::to_napi_object(block, env)?;
        ...
        from_wrapped.push(quote! {
            #key => {
                ...
                let block = <#fullpath as brec::napi_feat::NapiObject>::from_napi_object(&env, inner)?;
    ...
    Ok(quote! {
        ...
        impl brec::napi_feat::NapiObject for Block {
            fn to_napi_object<'env>(&self, env: &'env napi::Env) -> Result<napi::Unknown<'env>, brec::napi_feat::NapiError> {
