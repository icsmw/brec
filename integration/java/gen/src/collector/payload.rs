        ...
        if payload.attrs.is_bincode() {
            to_wrapped.push(quote! {
                Payload::#fullname(payload) => {
                    let value = brec::java_feat::JavaObject::to_java_object(payload, env)?;
            ...
            from_wrapped.push(quote! {
                #key => {
                    ...
                    let payload = <#fullpath as brec::java_feat::JavaObject>::from_java_object(env, inner)?;
        ...
        (
            quote! {
                Payload::Bytes(payload) => {
                    let value = <Vec<u8> as brec::java_feat::JavaConvert>::to_java_value(payload, env)?;
                    brec::java_feat::map_put(env, &obj, "Bytes", &value).map_err(|err| {
                        brec::java_feat::JavaError::InvalidAggregatorShape(err.to_string())
                    })?;
                }
                Payload::String(payload) => {
                    let value = <String as brec::java_feat::JavaConvert>::to_java_value(payload, env)?;
                    brec::java_feat::map_put(env, &obj, "String", &value).map_err(|err| {
                        brec::java_feat::JavaError::InvalidAggregatorShape(err.to_string())
                    })?;
                }
            },
            quote! {
                "Bytes" => {
                    let inner = brec::java_feat::map_get(env, &obj, "Bytes").map_err(|err| {
                        brec::java_feat::JavaError::InvalidAggregatorShape(err.to_string())
                    })?;
                    let payload = <Vec<u8> as brec::java_feat::JavaConvert>::from_java_value(env, inner)?;
                    return Ok(Payload::Bytes(payload));
                }
                "String" => {
                    let inner = brec::java_feat::map_get(env, &obj, "String").map_err(|err| {
                        brec::java_feat::JavaError::InvalidAggregatorShape(err.to_string())
                    })?;
                    let payload = <String as brec::java_feat::JavaConvert>::from_java_value(env, inner)?;
