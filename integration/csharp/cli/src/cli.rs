use crate::{BindingsCrate, CSharpPackage, Error, Model};
use std::env;
use std::fs;
...
/// prepares a standalone Rust bindings crate, builds the native artifact, and
/// writes a C# project that consumes that artifact through P/Invoke.
pub fn run() -> Result<(), Error> {
    ...
    let package_dir = cli.out.unwrap_or_else(|| scheme_dir.join("csharp"));
    ...
    CSharpPackage::new(&package_dir, &model, binding_artifact).write()?;

    println!(
        "generated C# package from {} into {}",
        scheme_path.display(),
        package_dir.display()
    );
    Ok(())
    ...
    fn parse(args: impl Iterator<Item = String>) -> Result<Self, Error> {
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--csharp-out" => {
                    let value = next_value(&mut args, "--csharp-out")?;
                    out = Some(PathBuf::from(value));
                }
                "--bindings-out" => {
...
fn print_usage() {
    println!(
        "Usage: brec_csharp_cli [--scheme <PATH>] [--out <DIR>] [--bindings-out <DIR>] [--protocol <DIR>] [--cargo-deps <PATH>]
...
By default, the generated bindings crate and C# project are written next to
the scheme file. --out and --csharp-out are aliases for the C# output directory.
--cargo-deps overrides built-in Rust dependency versions by name."
    );
}
...
mod tests {
    ...
    fn parses_all_paths() {
        ...
        assert_eq!(cli.out, Some(PathBuf::from("generated/csharp")));
