use crate::Error;
use brec_scheme::{PayloadTy, SchemeFieldType, SchemeFile, SchemePayloadField};
...
impl TypeNames {
    ...
    fn referenced_names(scheme: &SchemeFile) -> Vec<String> {
        ...
        for scheme_type in &scheme.types {
            Self::collect_fields(&scheme_type.fields, &mut refs);
    ...
    fn collect_fields(fields: &[SchemePayloadField], refs: &mut BTreeSet<String>) {
        for field in fields {
            if let SchemeFieldType::Payload(ty) = &field.ty {
