...
mkdir -p "${ROOT_DIR}/client/native"
case "${BINDINGS_LIB}" in
  *.so) cp "${BINDINGS_LIB}" "${ROOT_DIR}/client/native/libbindings.so" ;;
  *.dylib) cp "${BINDINGS_LIB}" "${ROOT_DIR}/client/native/libbindings.dylib" ;;
  *.dll) cp "${BINDINGS_LIB}" "${ROOT_DIR}/client/native/bindings.dll" ;;
esac
