...
echo "Rebuilding binding/pkg via wasm-pack (target=nodejs)..."
(
  cd "${WASM_DIR}/binding"
  wasm-pack build --dev --target nodejs --out-dir pkg --out-name wasmjs
)
...
echo "Building wasm-node client image..."
docker build -t "${IMAGE_TAG}" -f "${WASM_DIR}/clients/node/Dockerfile" "${WASM_DIR}"

echo "Running wasm-node client in container..."
docker run --rm \
