...
E2E_JAVA_DIR="${ROOT_DIR}/e2e/java"
OUT_DIR="${E2E_JAVA_DIR}/coverage_results"
LCOV_OUT="${OUT_DIR}/lcov.info"
LCOV_DEP_OUT="${OUT_DIR}/lcov.dep.brec.info"
WORKSPACE_MANIFEST="${E2E_JAVA_DIR}/Cargo.toml"

mkdir -p "${OUT_DIR}"
rm -f "${LCOV_OUT}" "${LCOV_DEP_OUT}"

echo "Preparing coverage environment for e2e/java..."
...
echo "Cleaning old coverage artifacts for e2e/java workspace..."
cargo llvm-cov clean --workspace --manifest-path "${WORKSPACE_MANIFEST}"

echo "Running java e2e with coverage instrumentation..."
(
  cd "${E2E_JAVA_DIR}"
  BINDINGS_PROFILE=debug bash "./test.sh"
)

echo "Generating e2e/java lcov report..."
...
if [[ -s "${LCOV_DEP_OUT}" ]]; then
  if command -v lcov >/dev/null 2>&1; then
    echo "Merging dependency report into e2e/java lcov..."
...
echo "e2e/java coverage report generated:"
echo "  LCOV: ${LCOV_OUT}"
