...
GENERATED_JAVA_DIR="${ROOT_DIR}/generated/java"
...
echo "Generating Java bindings crate and Java sources..."
(
  cd "${ROOT_DIR}"
  cargo run --manifest-path "${ROOT_DIR}/../../Cargo.toml" -p brec_java_cli -- \
    --scheme "${SCHEME_PATH}" \
    --protocol "${ROOT_DIR}/protocol" \
    --bindings-out "${ROOT_DIR}/bindings" \
    --java-out "${GENERATED_JAVA_DIR}" \
