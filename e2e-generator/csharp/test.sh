...
SERVER_PID=""
TAIL_PID=""
IMAGE_TAG="csharp-e2e:local"
SCHEME_PATH="${ROOT_DIR}/protocol/target/brec.scheme.json"
GENERATED_CSHARP_DIR="${ROOT_DIR}/generated/csharp"
...
echo "Cleaning generated C# artifacts..."
rm -rf "${ROOT_DIR}/bindings" "${GENERATED_CSHARP_DIR}"
rm -f "${SCHEME_PATH}"
...
echo "Generating C# bindings crate and project..."
(
  cd "${ROOT_DIR}"
  CARGO_NET_OFFLINE=true \
  cargo run --manifest-path "${ROOT_DIR}/../../Cargo.toml" -p brec_csharp_cli -- \
    --scheme "${SCHEME_PATH}" \
    --protocol "${ROOT_DIR}/protocol" \
    --bindings-out "${ROOT_DIR}/bindings" \
    --csharp-out "${GENERATED_CSHARP_DIR}" \
...
echo "Building csharp client image..."
docker build -t "${IMAGE_TAG}" -f "${ROOT_DIR}/client/Dockerfile" "${ROOT_DIR}"

echo "Running csharp client in container..."
DOCKER_RUN_ARGS=(
  --rm
