...
// impl<'a> BlockRef<'a> {
//     pub fn test_read<T: std::io::Read>(buf: &'a mut T) -> Result<Self, brec::Error>
//     where
//         Self: Sized,
//     {
...
//         let mut reader = std::io::BufReader::new(buf);
//         let bytes = reader.fill_buf()?;
//         match BlockAReferred::read_from_slice(bytes, false) {
//             Ok(blk) => return Ok(BlockRef::BlockA(blk)),
//             Err(err) => {
...
//             }
//         }
//         match BlockCReferred::read_from_slice(bytes, false) {
//             Ok(blk) => return Ok(BlockRef::BlockC(blk)),
//             Err(err) => {
//                 if !matches!(err, brec::Error::SignatureDismatch) {
//                     return Err(err);
//                 }
...
impl brec::Size for BlockRef<'_> {
    fn size(&self) -> u64 {
        match self {
            BlockRef::BlockA(..) => BlockA::ssize(),
            BlockRef::BlockC(..) => BlockC::ssize(),
        }
    }
}
