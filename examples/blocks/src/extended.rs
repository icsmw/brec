...
impl<'a> From<CustomBlockReferred<'a>> for CustomBlock {
    fn from(block: CustomBlockReferred<'a>) -> Self {
        CustomBlock {
            ...
            blob_a: *block.blob_a,
            field_a_i64: block.field_a_i64,
            field_a_i128: block.field_a_i128,
            blob_b: *block.blob_b,
        }
    }
}
const CUSTOMBLOCK: [u8; 4] = [54u8, 16u8, 49u8, 252u8];
impl brec::SignatureU32 for CustomBlockReferred<'_> {
    fn sig() -> &'static [u8; 4] {
        &CUSTOMBLOCK
    }
}
impl brec::CrcU32 for CustomBlock {
    fn crc(&self) -> [u8; 4] {
        ...
        hasher.update(&self.field_a_i64.to_le_bytes());
        hasher.update(&self.field_a_i128.to_le_bytes());
        hasher.update(&self.blob_b);
        hasher.finalize().to_le_bytes()
    }
}
impl brec::CrcU32 for CustomBlockReferred<'_> {
    fn crc(&self) -> [u8; 4] {
        let mut hasher = brec::crc32fast::Hasher::new();
        hasher.update(&[self.field_u8]);
        hasher.update(&self.field_u16.to_le_bytes());
...
impl brec::ReadBlockFrom for CustomBlock {
        ...
        Self: Sized,
    {
        ...
        let mut field_a_i64 = [0u8; 8usize];
        buf.read_exact(&mut field_a_i64)?;
        let field_a_i64 = i64::from_le_bytes(field_a_i64);
        let mut field_a_i128 = [0u8; 16usize];
        buf.read_exact(&mut field_a_i128)?;
        let field_a_i128 = i128::from_le_bytes(field_a_i128);
...
impl<'a> brec::ReadBlockFromSlice<'a> for CustomBlockReferred<'a> {
        ...
        Self: Sized,
    {
        ...
        let field_a_i64 = i64::from_le_bytes(buf[179usize..187usize].try_into()?);
        let field_a_i128 = i128::from_le_bytes(buf[187usize..203usize].try_into()?);
        let blob_b = <&[u8; 100usize]>::try_from(&buf[203usize..303usize])?;
        let __crc = <&[u8; 4usize]>::try_from(&buf[303usize..303usize + 4usize])?;
        let crc = __crc;
...
impl brec::TryReadFrom for CustomBlock {
...
impl brec::TryReadFromBuffered for CustomBlock {
...
impl brec::WriteTo for CustomBlock {
    fn write<T: std::io::Write>(&self, writer: &mut T) -> std::io::Result<usize> {
        ...
        offset += 100usize;
        buffer[offset..offset + 8usize].copy_from_slice(&self.field_a_i64.to_le_bytes());
        offset += 8usize;
        buffer[offset..offset + 16usize].copy_from_slice(&self.field_a_i128.to_le_bytes());
        offset += 16usize;
        unsafe {
            let dst = buffer.as_mut_ptr().add(offset);
...
impl brec::WriteVectoredTo for CustomBlock {
    fn slices(&self) -> std::io::Result<brec::IoSlices> {
        use brec::prelude::*;
        let mut slices = brec::IoSlices::default();
        slices.add_buffered(CUSTOMBLOCK.to_vec());
        let mut buffer = [0u8; 75usize];
        let mut offset = 0;
        buffer[offset..offset + 1usize].copy_from_slice(&[self.field_u8]);
        offset += 1usize;
        buffer[offset..offset + 2usize].copy_from_slice(&self.field_u16.to_le_bytes());
        ...
        slices.add_slice(&self.blob_a);
        let mut buffer = [0u8; 24usize];
        let mut offset = 0;
        buffer[offset..offset + 8usize].copy_from_slice(&self.field_a_i64.to_le_bytes());
        offset += 8usize;
        buffer[offset..offset + 16usize].copy_from_slice(&self.field_a_i128.to_le_bytes());
